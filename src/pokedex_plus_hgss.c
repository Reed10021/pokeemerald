...
static const u8 sEvoMap_ShoalIceCave[] = _("SHOAL ICE CAVE");
...
#define HGSS_ENABLE_FORMS_PAGE 1
...
struct EvoScreenData
{
    ...
    u8 menuPos;
    u8 arrowSpriteId;
    u16 arrowSpriteDist[10];
    #ifdef POKEMON_EXPANSION
    bool8 isMega;
    #endif
};

#if HGSS_ENABLE_FORMS_PAGE
...
static void PrintInfoScreenTextSmallFont(const u8* str, u8 fontId, u8 left, u8 top);
...
static void PrintEvolutionTargetSpeciesAndMethod(u8 taskId, u16 species, u8 depth, u32 *rowIndex, u16 alreadyPrintedIcons[], u32 *iconDisplayIndex, u32 *extraYOffset);
static u8 PrintPreEvolutions(u8 taskId, u16 species);
static void HandleTargetSpeciesPrintText(u16 targetSpecies, u8 base_x, u8 base_y, u8 base_y_offset, u8 row, u32 extraYOffset);
static void HandleTargetSpeciesPrintIcon(u8 taskId, u16 targetSpecies, u8 displayIndex, u8 iterations);
static bool8 DexStringHasManualBreaks(const u8 *str);
static u32 CountDexLineBreaks(const u8 *str);
static void BreakDexStringByWidth(u8 *str, u32 maxWidth, u8 fontId);
...
static u16 CreateMonSpriteFromSpecies(u16 species, s16 x, s16 y, u16 paletteSlot, bool8 shiny);
static u16 NormalizeToBaseFormSpecies(u16 species);
static bool8 SpeciesHasDexForms(u16 species);
static u16 GetDexFormSpeciesId(u16 species, u8 formId);

//HGSS_UI Physical Special icons 
static u8 ShowSplitIcon(u8 split); //Physical/Special Split from BE
...
static void ResetPokedexView(struct PokedexView *pokedexView)
{
    ...
    pokedexView->sEvoScreenData.fromEvoPage = FALSE;
...
{
    if (gSprites[sPokedexView->selectedMonSpriteId].pos1.x == MON_PAGE_X && gSprites[sPokedexView->selectedMonSpriteId].pos1.y == MON_PAGE_Y)
    {
...
void SpriteCB_MoveMonForInfoScreen(struct Sprite *sprite)
{
    ...
    sprite->pos2.x = 0;
    sprite->pos2.y = 0;
    if (sprite->pos1.x != MON_PAGE_X || sprite->pos1.y != MON_PAGE_Y)
    {
        if (sprite->pos1.x > 48)
            sprite->pos1.x -= 4;
        if (sprite->pos1.x < 48)
...
static void Task_LoadInfoScreen(u8 taskId)
{
    ...
    case 5:
        if (!gTasks[taskId].tMonSpriteDone)
        {
            ...
            if (sPokedexView->pokedexList[sPokedexView->selectedPokemon].seen)
                gTasks[taskId].tMonSpriteId = (u16)CreateMonSpriteFromSpecies(species, MON_PAGE_X, MON_PAGE_Y, 0, FALSE);
            else
                gTasks[taskId].tMonSpriteId = (u16)CreateMonSpriteFromSpecies(SPECIES_NONE, MON_PAGE_X, MON_PAGE_Y, 0, FALSE);
    ...
    case 8:
        ...
        {
            if (sPokedexView->pokedexList[sPokedexView->selectedPokemon].seen)
                PlayCry1(NationalPokedexNumToSpeciesHGSS(sPokedexListItem->dexNum), 0);
    ...
    if (JOY_NEW(A_BUTTON))
    {
        if (gTasks[taskId].tShinyFlag == 0)
        {
            if (sPokedexView->pokedexList[sPokedexView->selectedPokemon].seen)
            {
                ...
                gTasks[taskId].tMonSpriteId = (u16)CreateMonSpriteFromSpecies(species, MON_PAGE_X, MON_PAGE_Y, 0, TRUE);
                gSprites[gTasks[taskId].tMonSpriteId].oam.priority = 0;
...
static void Task_DisplayCaughtMonDexPage(u8 taskId)
{
    ...
    case 4:
        ...
        if (shinyValue < SHINY_ODDS)
            spriteId = CreateMonSpriteFromSpecies(species, MON_PAGE_X, MON_PAGE_Y, 0, TRUE);
        else
            spriteId = CreateMonSpriteFromSpecies(species, MON_PAGE_X, MON_PAGE_Y, 0, FALSE);
    ...
    case 6:
        if (!gPaletteFade.active)
        {
            PlayCry1(NationalPokedexNumToSpeciesHGSS(dexNum), 0);
...
static void PrintInfoScreenTextSmall(const u8* str, u8 left, u8 top)
{
    PrintInfoScreenTextSmallFont(str, 0, left, top);
}
static void PrintInfoScreenTextSmallWhite(const u8* str, u8 left, u8 top)
{
    u8 color[3];
...
static bool8 DexStringHasManualBreaks(const u8 *str)
{
    while (*str != EOS)
    {
        if (*str == CHAR_NEWLINE || *str == CHAR_PROMPT_SCROLL)
            return TRUE;
        str++;
    }
...
static void BreakDexStringByWidth(u8 *str, u32 maxWidth, u8 fontId)
{
    ...
    u32 spaceWidth;
    u32 length;
    bool8 firstWordOnLine;
    u8 spaceString[2] = {CHAR_SPACE, EOS};
    u8 *srcCopy;

    if (DexStringHasManualBreaks(str))
        return;
    ...
    srcCopy = Alloc(length + 1);
    if (srcCopy == NULL)
        return;

    StringCopy(srcCopy, str);
    spaceWidth = GetStringWidth(fontId, spaceString, 0);
    srcIndex = 0;
    dstIndex = 0;
    lineWidth = 0;
    firstWordOnLine = TRUE;

    while (srcCopy[srcIndex] != EOS)
    {
        while (srcCopy[srcIndex] == CHAR_SPACE)
            srcIndex++;

        if (srcCopy[srcIndex] == EOS)
            break;

        wordStart = srcIndex;
        while (srcCopy[srcIndex] != EOS && srcCopy[srcIndex] != CHAR_SPACE)
            srcIndex++;

        wordLength = srcIndex - wordStart;
        if (wordLength == 0)
            continue;

        {
            u8 savedChar = srcCopy[srcIndex];
            srcCopy[srcIndex] = EOS;
            wordWidth = GetStringWidth(fontId, &srcCopy[wordStart], 0);
            srcCopy[srcIndex] = savedChar;
        }

        if (!firstWordOnLine && lineWidth + spaceWidth + wordWidth > maxWidth)
        {
            str[dstIndex++] = CHAR_NEWLINE;
            lineWidth = 0;
            firstWordOnLine = TRUE;
        }
        ...
        if (!firstWordOnLine)
        {
            str[dstIndex++] = CHAR_SPACE;
            lineWidth += spaceWidth;
        }

        memcpy(&str[dstIndex], &srcCopy[wordStart], wordLength);
        dstIndex += wordLength;
        lineWidth += wordWidth;
...
static u8 GetUnownFormIdFromSpecies(u16 species)
{
    if (species >= SPECIES_UNOWN_B && species <= SPECIES_UNOWN_QMARK)
        return species - SPECIES_UNOWN_B + 1;
    return 0;
}

static u32 GetUnownPersonalityFromFormId(u8 formId)
{
    u32 personality = 0;

    personality |= (formId & 3);
    personality |= ((formId >> 2) & 3) << 8;
    personality |= ((formId >> 4) & 3) << 16;
    personality |= ((formId >> 6) & 3) << 24;
    return personality;
}
...
static u16 CreateMonSpriteFromSpecies(u16 species, s16 x, s16 y, u16 paletteSlot, bool8 shiny)
{
    ...
    // Unown forms in this repo are represented as alternate IDs above NUM_SPECIES.
    // Keep data lookups on base Unown, but override personality so the chosen letter renders.
    if (species == SPECIES_UNOWN
        && sPokedexView != NULL
        && sPokedexView->formSpecies >= SPECIES_UNOWN_B
        && sPokedexView->formSpecies <= SPECIES_UNOWN_QMARK)
    {
        personality = GetUnownPersonalityFromFormId(GetUnownFormIdFromSpecies(sPokedexView->formSpecies));
    }

    // Force shiny/non-shiny deterministically for the current personality.
    if (shiny)
        otId = personality;
    else
        otId = personality ^ SHINY_ODDS;

    return CreateMonPicSprite_HandleDeoxys(species, otId, personality, TRUE, x, y, paletteSlot, 0xFFFF);
}
...
static bool8 SpeciesHasDexForms(u16 species)
{
    species = NormalizeToBaseFormSpecies(species);
    return species == SPECIES_DEOXYS || species == SPECIES_UNOWN;
}

static u16 GetDexFormSpeciesId(u16 species, u8 formId)
{
    static const u16 sDexForms_Deoxys[] =
    {
        SPECIES_DEOXYS,
    ...
    static const u16 sDexForms_Unown[] =
    {
        SPECIES_UNOWN,
        SPECIES_UNOWN_B,
        SPECIES_UNOWN_C,
    ...
    const u16 *forms = NULL;
    u8 i;

    species = NormalizeToBaseFormSpecies(species);
    if (species == SPECIES_DEOXYS)
        forms = sDexForms_Deoxys;
    else if (species == SPECIES_UNOWN)
        forms = sDexForms_Unown;

    if (forms == NULL)
    {
        if (formId == 0)
            return species;
        return FORM_SPECIES_END;
    }
...
u16 NationalPokedexNumToSpeciesHGSS(u16 nationalNum)
{
    ...
    if (sPokedexView != NULL && sPokedexView->formSpecies != 0)
    {
        species = sPokedexView->formSpecies;
        if (species > NUM_SPECIES)
            return NormalizeToBaseFormSpecies(species);
        return species;
    }
    else
...
// Since we don't have the physical/special split, still show the physicality of the types.
...
static bool8 CalculateMoves(void)
{
    ...
    if (numEggMoves == 0) 
    {
        preEvolutionOne = GetPreEvolution(species);
        if(preEvolutionOne != 0) 
        {
            numEggMoves = GetEggMovesSpecies(preEvolutionOne, statsMovesEgg);
            if (numEggMoves == 0)
            {
                preEvolutionTwo = GetPreEvolution(preEvolutionOne);
                if (preEvolutionTwo != 0)
                {
                    numEggMoves = GetEggMovesSpecies(preEvolutionTwo, statsMovesEgg);
                }
            }
        }
    }
...
static void PrintStatsScreen_Moves_Top(u8 taskId)
{
    ...
    {
        PrintStatsScreenTextSmall(WIN_STATS_MOVES_TOP, gText_Stats_EggMove_Egg, moves_x + 113, moves_y + 3); //   Egg
        PrintStatsScreenTextSmall(WIN_STATS_MOVES_TOP, gText_Stats_EggMove_Move, moves_x + 113, moves_y + 14); // Move
        item = ITEM_LUCKY_EGG;
    }
    ...
    {
        PrintStatsScreenTextSmall(WIN_STATS_MOVES_TOP, gText_Stats_MoveTutor_Move, moves_x + 113, moves_y + 3); //   Move
        PrintStatsScreenTextSmall(WIN_STATS_MOVES_TOP, gText_Stats_MoveTutor_Tutor, moves_x + 113, moves_y + 14); // Tutor
        item = ITEM_TEACHY_TV;
    }
    else
    {
...
{
    if (differentEVs == 1 || differentEVs == 3 || differentEVs == 5)
        return 0;
    else
        return 1;
}
...
static u8 PrintMonStatsToggle_DifferentEVsRow(u8 differentEVs)
{
    if (differentEVs == 1 || differentEVs == 2)
        return 1;
    else if (differentEVs == 3 || differentEVs == 4)
        return 2;
    else if (differentEVs == 5 || differentEVs == 6)
        return 3;
}
...
static void PrintStatsScreen_Left(u8 taskId)
{
    ...
    u8 base_x_first_row_evs = 15;
    u8 base_x_second_row = 43;
    u8 base_x_second_row_evs = 40;
    u8 base_y_offset = 11;
    u32 base_i = 0;
    u32 base_y = 5;
    u32 align_x;
    ...
    if (gTasks[taskId].data[5] == 0)
    {
        PrintStatsScreenTextSmall(WIN_STATS_LEFT, gText_Stats_BaseStats, GetStringCenterAlignXOffset(0, gText_Stats_BaseStats, total_x+10), base_y + base_y_offset * base_i);
        base_i++;
    ...
    {
        PrintStatsScreenTextSmall(WIN_STATS_LEFT, gText_Stats_EvYield, GetStringCenterAlignXOffset(0, gText_Stats_EvYield, total_x+10), base_y + base_y_offset * base_i);
        //HP
        base_i++;
        PrintStatsScreenTextSmall(WIN_STATS_LEFT, gText_Stats_HP, base_x, base_y + base_y_offset * base_i);
        if (EVs[0] > 0)
        {
            PrintMonStatsToggle_EV_Arrows(gStringVar1, EVs[0]);
            PrintStatsScreenTextSmall(WIN_STATS_LEFT, gStringVar1, base_x + base_x_first_row_evs, base_y + base_y_offset * base_i);
        }
        //Speed
        PrintStatsScreenTextSmall(WIN_STATS_LEFT, gText_Stats_Speed, base_x + base_x_second_row_evs, base_y + base_y_offset * base_i);
        if (EVs[1] > 0)
        {
            PrintMonStatsToggle_EV_Arrows(gStringVar1, EVs[1]);
            PrintStatsScreenTextSmall(WIN_STATS_LEFT, gStringVar1, GetStringRightAlignXOffset(0, gStringVar1, total_x), base_y + base_y_offset * base_i);
        }
        ...
        if (EVs[2] > 0)
        {
            PrintMonStatsToggle_EV_Arrows(gStringVar1, EVs[2]);
            PrintStatsScreenTextSmall(WIN_STATS_LEFT, gStringVar1, base_x + base_x_first_row_evs, base_y + base_y_offset * base_i);
        }
        //Special Attack
        PrintStatsScreenTextSmall(WIN_STATS_LEFT, gText_Stats_SpAttack, base_x + base_x_second_row_evs, base_y + base_y_offset * base_i);
        if (EVs[3] > 0)
        {
            PrintMonStatsToggle_EV_Arrows(gStringVar1, EVs[3]);
            PrintStatsScreenTextSmall(WIN_STATS_LEFT, gStringVar1, GetStringRightAlignXOffset(0, gStringVar1, total_x), base_y + base_y_offset * base_i);
        }
        ...
        if (EVs[4] > 0)
        {
            PrintMonStatsToggle_EV_Arrows(gStringVar1, EVs[4]);
            PrintStatsScreenTextSmall(WIN_STATS_LEFT, gStringVar1, base_x + base_x_first_row_evs, base_y + base_y_offset * base_i);
        }
        //Special Defense
        PrintStatsScreenTextSmall(WIN_STATS_LEFT, gText_Stats_SpDefense, base_x + base_x_second_row_evs, base_y + base_y_offset * base_i);
        if (EVs[5] > 0)
        {
            PrintMonStatsToggle_EV_Arrows(gStringVar1, EVs[5]);
            PrintStatsScreenTextSmall(WIN_STATS_LEFT, gStringVar1, GetStringRightAlignXOffset(0, gStringVar1, total_x), base_y + base_y_offset * base_i);
        }
        base_i++;
    }
    ...
    if (gTasks[taskId].data[5] == 0)
    {
        ...
        if (catchRate <= 10) {
            ...
            PrintStatsScreenTextSmall(WIN_STATS_LEFT, gText_Stats_CatchRate_Legend, align_x, base_y + base_y_offset * base_i);
        }
        else if (catchRate <= 70) {
            align_x = GetStringRightAlignXOffset(0, gText_Stats_CatchRate_VeryHard, total_x);
            PrintStatsScreenTextSmall(WIN_STATS_LEFT, gText_Stats_CatchRate_VeryHard, align_x, base_y + base_y_offset * base_i);
        }
        else if (catchRate <= 100) {
            align_x = GetStringRightAlignXOffset(0, gText_Stats_CatchRate_Difficult, total_x);
            PrintStatsScreenTextSmall(WIN_STATS_LEFT, gText_Stats_CatchRate_Difficult, align_x, base_y + base_y_offset * base_i);
        }
        else if (catchRate <= 150) {
            align_x = GetStringRightAlignXOffset(0, gText_Stats_CatchRate_Medium, total_x);
            PrintStatsScreenTextSmall(WIN_STATS_LEFT, gText_Stats_CatchRate_Medium, align_x, base_y + base_y_offset * base_i);
        }
        else if (catchRate <= 200) {
            align_x = GetStringRightAlignXOffset(0, gText_Stats_CatchRate_Relaxed, total_x);
            PrintStatsScreenTextSmall(WIN_STATS_LEFT, gText_Stats_CatchRate_Relaxed, align_x, base_y + base_y_offset * base_i);
        }
        else {
            align_x = GetStringRightAlignXOffset(0, gText_Stats_CatchRate_Easy, total_x);
            PrintStatsScreenTextSmall(WIN_STATS_LEFT, gText_Stats_CatchRate_Easy, align_x, base_y + base_y_offset * base_i);
        }
        base_i++;
    ...
    else
    {
        ...
        if (sPokedexView->sPokemonStats.eggGroup1 != sPokedexView->sPokemonStats.eggGroup2)
        {
            ...
            // Check and see if it fits on same row
            align_x = GetStringRightAlignXOffset(0, gStringVar3, total_x - GetStringWidth(0, gText_Stats_EggGroups, 0));
            if (align_x != 0)
            {
                // If it does fit on same row, get the actual right align X offset and use that to print the text.
                PrintStatsScreenTextSmall(WIN_STATS_LEFT, gStringVar3, GetStringRightAlignXOffset(0, gStringVar3, total_x), base_y + base_y_offset * base_i);
            }
            else
            {
                // If it doesn't fit on same row, increment i and set it on a new line.
                base_i++;
        ...
        else // 1 egg group
        {
            ...
            // Check and see if it fits on same row
            align_x = GetStringRightAlignXOffset(0, gStringVar1, total_x - GetStringWidth(0, gText_Stats_EggGroup, 0));
            if (align_x != 0)
            {
                // If it does fit on same row, get the actual right align X offset and use that to print the text.
                PrintStatsScreenTextSmall(WIN_STATS_LEFT, gStringVar1, GetStringRightAlignXOffset(0, gStringVar1, total_x), base_y + base_y_offset * base_i);
...
static void PrintStatsScreen_Abilities(u8 taskId)
{
    ...
    u16 abilityHidden = sPokedexView->sPokemonStats.abilityHidden;

    //Abilitie(s)
    if (gTasks[taskId].data[5] == 0 || abilityHidden == ABILITY_NONE)
    {
...
static void ResetEvoScreenDataStruct(void)
{
    ...
    sPokedexView->sEvoScreenData.arrowSpriteId = SPRITE_NONE;
    for (i = 0; i < 10; i++)
    {
...
static void Task_LoadEvolutionScreen(u8 taskId)
{
    ...
    case 3:
        ...
        {
            u32 rowIndex;
            u32 iconDisplayIndex;
            u32 extraYOffset;
            u16 alreadyPrintedIcons[MAX_EVOLUTION_ICONS] = {0};
            ...
            rowIndex = sPokedexView->numPreEvolutions;
            iconDisplayIndex = sPokedexView->numPreEvolutions;
            extraYOffset = 0;
            PrintEvolutionTargetSpeciesAndMethod(taskId, NationalPokedexNumToSpeciesHGSS(sPokedexListItem->dexNum), 0, &rowIndex, alreadyPrintedIcons, &iconDisplayIndex, &extraYOffset);
        }
        gMain.state++;
        break;
    case 4:
        ...
        if (sPokedexView->sEvoScreenData.numAllEvolutions != 0 && sPokedexView->sEvoScreenData.numSeen != 0)
        {
            ...
            while (pos < sPokedexView->sEvoScreenData.numAllEvolutions && !sPokedexView->sEvoScreenData.seen[pos])
                pos++;

            if (pos >= sPokedexView->sEvoScreenData.numAllEvolutions)
                pos = 0;

            sPokedexView->sEvoScreenData.menuPos = pos;
            sPokedexView->sEvoScreenData.arrowSpriteId = CreateSprite(&sSpriteTemplate_Arrow, 7, 58 + sPokedexView->sEvoScreenData.arrowSpriteDist[pos] + 9 * pos, 0);
...
static void Task_HandleEvolutionScreenInput(u8 taskId)
{
    ...
    if (sPokedexView->sEvoScreenData.numAllEvolutions != 0 && sPokedexView->sEvoScreenData.numSeen != 0)
    {
        ...
        if (JOY_NEW(DPAD_DOWN))
        {
            ...
            gSprites[sPokedexView->sEvoScreenData.arrowSpriteId].pos1.y = base_y + sPokedexView->sEvoScreenData.arrowSpriteDist[pos] + base_y_offset * pos;
            sPokedexView->sEvoScreenData.menuPos = pos;
        }
...
#if HGSS_ENABLE_FORMS_PAGE
            if (SpeciesHasDexForms(targetSpecies))
...
static void HandleTargetSpeciesPrintText(u16 targetSpecies, u8 base_x, u8 base_y, u8 base_y_offset, u8 row, u32 extraYOffset)
{
    ...
    PrintInfoScreenTextSmall(gStringVar3, base_x, base_y + extraYOffset + base_y_offset * row); // evolution mon name
}
...
static void HandleTargetSpeciesPrintIcon(u8 taskId, u16 targetSpecies, u8 displayIndex, u8 iterations)
{
    ...
    if (iterations > 6)
        spriteId = CreateMonIcon(targetSpecies, SpriteCB_MonIcon, 45 + 26 * displayIndex, 31, 4, 0, TRUE); //Create pokemon sprite
    else
        spriteId = CreateMonIcon(targetSpecies, SpriteCB_MonIcon, 50 + 32 * displayIndex, 31, 4, 0, TRUE); //Create pokemon sprite

    gTasks[taskId].data[5 + gTasks[taskId].data[3]] = spriteId;
...
static void HandlePreEvolutionSpeciesPrint(u8 taskId, u16 preSpecies, u16 species, u8 base_x, u8 base_y, u8 base_y_offset, u8 base_i)
{
    ...
    if (base_i < 3)
    {
        ...
        spriteId = CreateMonIcon(preSpecies, SpriteCB_MonIcon, 18 + 32 * base_i, 31, 4, 0, TRUE); //Create pokemon sprite
        gTasks[taskId].data[5 + gTasks[taskId].data[3]] = spriteId;
        gTasks[taskId].data[3]++;
        gSprites[spriteId].oam.priority = 0;
    }
...
static void PrintEvolutionTargetSpeciesAndMethod(u8 taskId, u16 species, u8 depth, u32 *rowIndex, u16 alreadyPrintedIcons[], u32 *iconDisplayIndex, u32 *extraYOffset)
{
    ...
    bool32 seen;
        ...
        u16 j;
    #endif
    u16 displaySpecies = NormalizeToBaseFormSpecies(species);
    u16 targetSpecies;
    u16 item;
    u8 base_x = 13 + 8;
    u8 base_y = 51;
    ...
    if (times == 0 && depth == 0)
    {
        seen = GetSetPokedexFlag(SpeciesToNationalPokedexNum(displaySpecies), FLAG_GET_SEEN);
        if (seen || !HGSS_HIDE_UNSEEN_EVOLUTION_NAMES)
            StringCopy(gStringVar1, gSpeciesNames[displaySpecies]);
        else
            StringCopy(gStringVar1, gText_ThreeQuestionMarks);
        StringExpandPlaceholders(gStringVar4, gText_EVO_NONE);
        PrintInfoScreenTextSmall(gStringVar4, base_x - 7 - 7, base_y + *extraYOffset + base_y_offset * (*rowIndex));
        return;
    }
    ...
    for (i = 0; i < times; i++)
    {
        ...
        #endif
        sPokedexView->sEvoScreenData.targetSpecies[*rowIndex] = targetSpecies;
        sPokedexView->sEvoScreenData.arrowSpriteDist[*rowIndex] = *extraYOffset;
        CreateCaughtBallEvolutionScreen(targetSpecies, base_x + depth_x * depth - 9, base_y + *extraYOffset + base_y_offset * (*rowIndex), 0);
        HandleTargetSpeciesPrintText(targetSpecies, base_x + depth_x * depth, base_y, base_y_offset, *rowIndex, *extraYOffset);

        for (item = 0; item < MAX_EVOLUTION_ICONS; item++)
        {
            if (alreadyPrintedIcons[item] == targetSpecies)
                break;
            if (alreadyPrintedIcons[item] == SPECIES_NONE)
            {
                HandleTargetSpeciesPrintIcon(taskId, targetSpecies, *iconDisplayIndex, times);
                alreadyPrintedIcons[item] = targetSpecies;
                (*iconDisplayIndex)++;
                break;
            }
        }
        ...
        case EVO_IF_IN_MAP:
            ...
            if (mapHeader->mapLayoutId == LAYOUT_SHOAL_CAVE_LOW_TIDE_ICE_ROOM)
            {
                StringCopy(gStringVar2, sEvoMap_ShoalIceCave);
            }
            else
            { 
...

        {
            u8 fontId = 0;
            u8 row = *rowIndex;
            u32 methodTextX = base_x + depth_x * depth + GetStringWidth(fontId, gStringVar3, 0) + 6;
            u32 maxMethodWidth;
            u32 fontHeight = GetFontAttribute(fontId, FONTATTR_MAX_LETTER_HEIGHT);

            if (methodTextX >= 230)
                methodTextX = 229;
            maxMethodWidth = 230 - methodTextX;
            if (maxMethodWidth < 24)
                maxMethodWidth = 24;

            BreakDexStringByWidth(gStringVar4, maxMethodWidth, fontId);
            PrintInfoScreenTextSmallFont(gStringVar4, fontId, methodTextX, base_y + *extraYOffset + base_y_offset * row); //Print actual instructions

            (*rowIndex)++;
            *extraYOffset += CountDexLineBreaks(gStringVar4) * fontHeight;
        }

        PrintEvolutionTargetSpeciesAndMethod(taskId, targetSpecies, depth + 1, rowIndex, alreadyPrintedIcons, iconDisplayIndex, extraYOffset);
    }//For loop end
}
...
static void Task_SwitchScreensFromEvolutionScreen(u8 taskId)
{
    ...
    if (!gPaletteFade.active)
    {
        if (sPokedexView->sEvoScreenData.arrowSpriteId != SPRITE_NONE)
            DestroySprite(&gSprites[sPokedexView->sEvoScreenData.arrowSpriteId]);
        FreeMonIconPalettes();                                          //Destroy pokemon icon sprite
        FreeAndDestroyMonIconSprite(&gSprites[gTasks[taskId].data[4]]); //Destroy pokemon icon sprite
...
static void Task_LoadFormsScreen(u8 taskId)
{
    ...
    case 3:
        if (gTasks[taskId].data[1] == 0)
        {
            u16 currentFormSpecies = NationalPokedexNumToSpeciesHGSS(sPokedexListItem->dexNum);

            if (sPokedexView->formSpecies != 0
                && NormalizeToBaseFormSpecies(sPokedexView->formSpecies) == NormalizeToBaseFormSpecies(currentFormSpecies))
            {
                currentFormSpecies = sPokedexView->formSpecies;
            }

            //Icon
            FreeMonIconPalettes(); //Free space for new pallete
            if (NormalizeToBaseFormSpecies(currentFormSpecies) == SPECIES_UNOWN)
            {
                u32 personality = GetUnownPersonalityFromFormId(GetUnownFormIdFromSpecies(currentFormSpecies));

                LoadMonIconPalette(SPECIES_UNOWN);
                gTasks[taskId].data[4] = CreateMonIcon(SPECIES_UNOWN, SpriteCB_MonIcon, 18, 31, 4, personality, TRUE); //Create pokemon sprite
            }
            else
            {
                LoadMonIconPalette(currentFormSpecies);
                gTasks[taskId].data[4] = CreateMonIcon(currentFormSpecies, SpriteCB_MonIcon, 18, 31, 4, 0, TRUE); //Create pokemon sprite
            }
            gSprites[gTasks[taskId].data[4]].oam.priority = 0;
        }
        {
            ...
            for (i = 0; i < ARRAY_COUNT(sPokedexView->sFormScreenData.iconSpriteIds); i++)
                sPokedexView->sFormScreenData.iconSpriteIds[i] = SPRITE_NONE;
        }
        sPokedexView->sFormScreenData.inSubmenu = TRUE;
        sPokedexView->sFormScreenData.numForms = 0;
        sPokedexView->sFormScreenData.menuPos = 1;
        sPokedexView->sFormScreenData.arrowSpriteId = SPRITE_NONE;
        EvoFormsPage_PrintNavigationButtons(); //HGSS_Ui Navigation buttons
        gMain.state++;
        break;
    case 4:
        {
            ...
            if (sPokedexView->sFormScreenData.numForms != 0)
            {
                menuPos = sPokedexView->sFormScreenData.menuPos - 1;
                row = menuPos / 7;
                column = menuPos % 7;
                sPokedexView->sFormScreenData.arrowSpriteId = CreateSprite(&sSpriteTemplate_Arrow, 5 + 34 * column, 34 + 34 * row, 0);
...
static void Task_HandleFormsScreenInput(u8 taskId)
{
    ...
    if (sPokedexView->sFormScreenData.numForms != 0)
    {
        ...
        row = (menuPos - 1) / 7;
        column = (menuPos - 1) % 7;
        gSprites[sPokedexView->sFormScreenData.arrowSpriteId].pos1.x = base_x + offset_x * column;
        gSprites[sPokedexView->sFormScreenData.arrowSpriteId].pos1.y = base_y + offset_y * row;
        ...
        if (JOY_NEW(A_BUTTON))
        {
            ...
            u16 formSpecies = GetDexFormSpeciesId(NationalPokedexNumToSpeciesHGSS(sPokedexListItem->dexNum), formId);
            if (sPokedexView->isSearchResults && sPokedexView->originalSearchSelectionNum == 0)
                sPokedexView->originalSearchSelectionNum = sPokedexListItem->dexNum;
            if (formSpecies == NormalizeToBaseFormSpecies(formSpecies))
                sPokedexView->formSpecies = 0;
            else
...
static void PrintForms(u8 taskId, u16 species)
{
    ...
    if (sPokedexView != NULL
        && sPokedexView->formSpecies != 0
        && NormalizeToBaseFormSpecies(sPokedexView->formSpecies) == species)
    {
        selectedSpecies = sPokedexView->formSpecies;
    }

    if (species == SPECIES_UNOWN)
        y_offset_icons = 8;

    if (GetDexFormSpeciesId(species, 0) == SPECIES_UNOWN)
        y_offset_icons = 8;
    ...
    for (i=0; i < 30; i++)
    {
        speciesForm = GetDexFormSpeciesId(species, i);
        if (speciesForm == FORM_SPECIES_END)
            break;

        totalForms += 1;
        sPokedexView->sFormScreenData.formIds[totalForms] = i;

        if (speciesForm == selectedSpecies)
            selectedMenuPos = totalForms;

        if (speciesForm != species)
        {
            u8 spriteId;

            times += 1;
            if (NormalizeToBaseFormSpecies(speciesForm) == SPECIES_UNOWN)
            {
                u32 personality = GetUnownPersonalityFromFormId(GetUnownFormIdFromSpecies(speciesForm));

                LoadMonIconPalette(SPECIES_UNOWN); //Loads palette for current mon
                if (times < 7)
                    spriteId = CreateMonIcon(SPECIES_UNOWN, SpriteCB_MonIcon, 52 + 34*(times-1), 31, 4, personality, TRUE);
                else if (times < 14)
                    spriteId = CreateMonIcon(SPECIES_UNOWN, SpriteCB_MonIcon, 18 + 34*(times-7), 70 - y_offset_icons, 4, personality, TRUE);
                else if (times < 21)
                    spriteId = CreateMonIcon(SPECIES_UNOWN, SpriteCB_MonIcon, 18 + 34*(times-14), 104 - y_offset_icons, 4, personality, TRUE);
                else
                    spriteId = CreateMonIcon(SPECIES_UNOWN, SpriteCB_MonIcon, 18 + 34*(times-21), 138 - y_offset_icons, 4, personality, TRUE);
            }
            else
            {
                LoadMonIconPalette(speciesForm); //Loads palette for current mon
                if (times < 7)
                    spriteId = CreateMonIcon(speciesForm, SpriteCB_MonIcon, 52 + 34*(times-1), 31, 4, 0, TRUE);
                else if (times < 14)
                    spriteId = CreateMonIcon(speciesForm, SpriteCB_MonIcon, 18 + 34*(times-7), 70 - y_offset_icons, 4, 0, TRUE);
                else if (times < 21)
                    spriteId = CreateMonIcon(speciesForm, SpriteCB_MonIcon, 18 + 34*(times-14), 104 - y_offset_icons, 4, 0, TRUE);
                else
                    spriteId = CreateMonIcon(speciesForm, SpriteCB_MonIcon, 18 + 34*(times-21), 138 - y_offset_icons, 4, 0, TRUE);
            }
            sPokedexView->sFormScreenData.iconSpriteIds[times] = spriteId; //Create pokemon sprite
            gSprites[spriteId].oam.priority = 0;
        }
    }
    gTasks[taskId].data[3] = times;
    sPokedexView->sFormScreenData.numForms = totalForms;
    sPokedexView->sFormScreenData.menuPos = selectedMenuPos;
...
static void Task_SwitchScreensFromFormsScreen(u8 taskId)
{
    ...
    if (!gPaletteFade.active)
    {
        if (sPokedexView->sFormScreenData.arrowSpriteId != SPRITE_NONE)
            DestroySprite(&gSprites[sPokedexView->sFormScreenData.arrowSpriteId]);
        FreeMonIconPalettes();                                          //Destroy pokemon icon sprite
        FreeAndDestroyMonIconSprite(&gSprites[gTasks[taskId].data[4]]); //Destroy pokemon icon sprite
        for (i = 1; i <= gTasks[taskId].data[3]; i++)
        {
            u8 iconSpriteId = sPokedexView->sFormScreenData.iconSpriteIds[i];

            if (iconSpriteId != SPRITE_NONE)
                FreeAndDestroyMonIconSprite(&gSprites[iconSpriteId]); //Destroy pokemon icon sprite
        }
        FreeAndDestroyMonPicSprite(gTasks[taskId].tMonSpriteId);
...
static void Task_LoadCryScreen(u8 taskId)
{
    ...
    case 4:
        ...
        break;
    case 5:
        gTasks[taskId].tMonSpriteId = CreateMonSpriteFromSpecies(NationalPokedexNumToSpeciesHGSS(sPokedexListItem->dexNum), MON_PAGE_X, MON_PAGE_Y, 0, FALSE);
        gSprites[gTasks[taskId].tMonSpriteId].oam.priority = 0;
...
static void Task_LoadSizeScreen(u8 taskId)
{
    ...
    case 5:
        ...
        break;
    case 6:
        spriteId = CreateMonSpriteFromSpecies(NationalPokedexNumToSpeciesHGSS(sPokedexListItem->dexNum), 88, 56, 1, FALSE);
        gSprites[spriteId].oam.affineMode = ST_OAM_AFFINE_NORMAL;
