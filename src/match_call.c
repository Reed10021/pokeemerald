...
static bool32 SelectMatchCallTrainer(void)
{
   ...
   || sub_8196D74(matchCallId))) //ShouldTrainerRequestBattle
        return FALSE;
...
static u16 GetFrontierStreakInfo(u16 facilityId, u32 *topicTextId)
{
    ...
    case FRONTIER_FACILITY_FACTORY:
        for (i = 0; i < 2; i++)
        {
            // BUG: (notabug) should be looking at battle factory records.
    ...
    case FRONTIER_FACILITY_PIKE:
        for (i = 0; i < 2; i++)
        {
            for (j = 0; j < 2; j++)
            {
                // BUG: (notabug) should be looking at battle pike records.
