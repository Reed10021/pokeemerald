...
static const u16 sRegionMap_SpecialPlaceLocations[][2] =
{
    ...
    //{MAPSEC_UNDERWATER_125,             MAPSEC_ROUTE_129}, // BUG: (fixed) Map will incorrectly display the name of Route 129 when diving on Route 125 (for Marine Cave only)
...
u8 *GetMapName(u8 *dest, u16 regionMapId, u16 padLength)
{
    ...
    else if (regionMapId < MAPSEC_NONE)
    {
        if (regionMapId == MAPSEC_ALTERING_CAVE) {
            ...
            *str++ = CHAR_SPACE;
            *str++ = (u8)CHAR_0 + (VarGet(VAR_ALTERING_CAVE_WILD_SET) + 1); // 161 or A1 or '0'
            *str++ = EOS;
        } else {
            str = StringCopy(dest, gRegionMapEntries[regionMapId].name);
        }
