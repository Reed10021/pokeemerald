...
static u16 GetDomeCardMove(u16 trainerId, u8 trainerTourneyId, u8 monId, u8 moveSlot);
static void AccumulateDomeMovePoints(u16 move, s16 *allocatedArray);
static u8 CalculateDomeBattleStyleFromPoints(const s16 *movePoints);
static u8 CalculateDomeTrainerBattleStyle(u8 trainerTourneyId);
static void CacheDomeTrainerBattleStyles(void);
static void EnsureDomeTrainerBattleStyles(void);
static bool32 IsDomeComboMove(u16 move);
static bool32 IsDomeDamagingMove(u16 move);
static bool32 IsDomeHealingMove(u16 move);
static bool32 IsDomeRiskyMove(u16 move);
static bool32 IsDomeStatusMove(u16 move);
static bool32 IsDomeDefensiveMove(u16 move);
static bool32 IsDomePopularMove(u16 move);
static bool32 IsDomeLuckyMove(u16 move);
static bool32 IsDomeRareMove(u16 move);
static bool32 IsDomeEffectMove(u16 move);
static bool32 IsStatRaisingEffect(u8 effect);
static bool32 IsStatLoweringEffect(u8 effect);
...
static void BeginPreviousDomeTourneyView(void);
static void EndPreviousDomeTourneyView(void);
...
static u8 sDomeTrainerBattleStyles[DOME_TOURNAMENT_TRAINERS_COUNT];
static bool8 sDomeTrainerBattleStylesValid;
static bool8 sPreviousDomeTourneyStateActive;
static u8 sPreviousDomeTourneyLvlMode;
static u8 sPreviousDomeTourneyBattleNum;

static u16 GetDomeCardMove(u16 trainerId, u8 trainerTourneyId, u8 monId, u8 moveSlot)
{
    if (trainerId == TRAINER_FRONTIER_BRAIN)
        return GetFrontierBrainMonMove(monId, moveSlot);
    else if (trainerId == TRAINER_PLAYER)
        return gSaveBlock2Ptr->frontier.domePlayerPartyData[monId].moves[moveSlot];
    else
        return gFacilityTrainerMons[DOME_MONS[trainerTourneyId][monId]].moves[moveSlot];
}
...
static bool32 IsDomeHealingMove(u16 move)
{
    // The Dome style text treats sustain fairly broadly: direct recovery,
    // draining, Rest/Wish-style reset, and self-cleansing like Refresh.
    switch (gBattleMoves[move].effect)
    {
    case EFFECT_ABSORB:
    case EFFECT_DREAM_EATER:
...
{
    u32 moveSlot;
    for (moveSlot = 0; moveSlot < NUM_TECHNICAL_MACHINES + NUM_HIDDEN_MACHINES; moveSlot++)
    {
        u16 foundMove = ItemIdToBattleMoveId(ITEM_TM01 + moveSlot);
        if (foundMove != MOVE_NONE && foundMove < MOVES_COUNT && foundMove == move)
        {
...
static bool32 IsDomeLuckyMove(u16 move)
{
    if (gBattleMoves[move].accuracy != 0 && gBattleMoves[move].accuracy <= 50)
        return TRUE;
...
static bool32 IsDomeEffectMove(u16 move)
{
    // This bucket is the most "flavor" driven one. Start with the
    // shared Sheer Force-style "extra effect" moves, then layer on the
    // non-Sheer-Force utility and drawback cases the Dome still calls out.
    if (IsSheerForceMove(move))
        return TRUE;
    ...
    case EFFECT_CONDITIONAL_DOUBLE_DAMAGE_HIT:
    case EFFECT_ALWAYS_CRIT_HIT:
    case EFFECT_POLLEN_PUFF:
        return TRUE;
    default:
        return FALSE;
    }
...
static bool32 IsDomeRareMove(u16 move)
{
    ...
    {
        const struct LevelUpMove *learnset = gLevelUpLearnsets[i];
        ...
        if ((species * 100) >= NUM_SPECIES) // If at least 1.0% of all mons or more can learn this move, return false.
            return FALSE;
    }
    return TRUE;
}
...
static void AccumulateDomeMovePoints(u16 move, s16 *allocatedArray)
{
    ...
    // Score each move directly from current move data so Dome cards stay in
    // sync with move reworks without maintaining a second source of truth.
    effect = gBattleMoves[move].effect;
    allocatedArray[MOVE_POINTS_COMBO] += IsDomeComboMove(move);
    allocatedArray[MOVE_POINTS_STAT_RAISE] += IsStatRaisingEffect(effect);
    allocatedArray[MOVE_POINTS_STAT_LOWER] += IsStatLoweringEffect(effect);
    allocatedArray[MOVE_POINTS_RARE] += IsDomeRareMove(move);
    allocatedArray[MOVE_POINTS_HEAL] += IsDomeHealingMove(move);
    allocatedArray[MOVE_POINTS_RISKY] += IsDomeRiskyMove(move);
    allocatedArray[MOVE_POINTS_STATUS] += IsDomeStatusMove(move);
    allocatedArray[MOVE_POINTS_DMG] += IsDomeDamagingMove(move);
    allocatedArray[MOVE_POINTS_DEF] += IsDomeDefensiveMove(move);
    allocatedArray[MOVE_POINTS_ACCURATE] += (gBattleMoves[move].accuracy == 0 || gBattleMoves[move].accuracy == 100);
    allocatedArray[MOVE_POINTS_POWERFUL] += (gBattleMoves[move].power >= 100);
    allocatedArray[MOVE_POINTS_POPULAR] += IsDomePopularMove(move);
    allocatedArray[MOVE_POINTS_LUCK] += IsDomeLuckyMove(move);
    allocatedArray[MOVE_POINTS_STRONG] += (gBattleMoves[move].power >= 90);
    allocatedArray[MOVE_POINTS_LOW_PP] += (gBattleMoves[move].pp <= 5);
    allocatedArray[MOVE_POINTS_EFFECT] += IsDomeEffectMove(move);
}
...
static const u8 sBattleStyleThresholds[NUM_BATTLE_STYLES - 1][NUM_MOVE_POINT_TYPES] =
{
    ...
    [DOME_BATTLE_STYLE_UNUSED1]         = {0}, // Sample messages below are intentionally excluded from the matcher.
...
// Search thresholds in an explicit priority order so more specific styles can
// resolve before their broader parent buckets without renumbering style ids.
static const u8 sBattleStyleMatchOrder[] =
{
...
static u8 CalculateDomeBattleStyleFromPoints(const s16 *movePoints)
{
    ...
    for (i = 0, battleStyle = DOME_BATTLE_STYLE_BALANCED; i < ARRAY_COUNT(sBattleStyleMatchOrder); i++)
    {
        u32 style = sBattleStyleMatchOrder[i];
        int metThresholdCount = 0;
        int thresholdStatCount = 0;

        for (j = 0; j < NUM_MOVE_POINT_TYPES; j++)
        {
            if (sBattleStyleThresholds[style][j] != 0)
            {
                thresholdStatCount++;
                if (movePoints[j] != 0 && movePoints[j] >= sBattleStyleThresholds[style][j])
                    metThresholdCount++;
            }
        }

        if (thresholdStatCount == metThresholdCount)
        {
            battleStyle = style;
            break;
        }
...
{
    int i, j;
    u16 trainerId = DOME_TRAINERS[trainerTourneyId].trainerId;
    s16 movePoints[NUM_MOVE_POINT_TYPES] = {0};

    for (i = 0; i < FRONTIER_PARTY_SIZE; i++)
    {
        for (j = 0; j < MAX_MON_MOVES; j++)
            AccumulateDomeMovePoints(GetDomeCardMove(trainerId, trainerTourneyId, i, j), movePoints);
    }

    return CalculateDomeBattleStyleFromPoints(movePoints);
}
...
static void CacheDomeTrainerBattleStyles(void)
{
    ...
    for (i = 0; i < DOME_TOURNAMENT_TRAINERS_COUNT; i++)
        sDomeTrainerBattleStyles[i] = CalculateDomeTrainerBattleStyle(i);

    sDomeTrainerBattleStylesValid = TRUE;
}

static void EnsureDomeTrainerBattleStyles(void)
{
    if (!sDomeTrainerBattleStylesValid)
        CacheDomeTrainerBattleStyles();
}

static void BeginPreviousDomeTourneyView(void)
{
    if (!sPreviousDomeTourneyStateActive)
    {
        sPreviousDomeTourneyLvlMode = gSaveBlock2Ptr->frontier.lvlMode;
        sPreviousDomeTourneyBattleNum = gSaveBlock2Ptr->frontier.curChallengeBattleNum;
        sPreviousDomeTourneyStateActive = TRUE;
    }
...
static void EndPreviousDomeTourneyView(void)
{
    ...
    gSaveBlock2Ptr->frontier.lvlMode = sPreviousDomeTourneyLvlMode;
    gSaveBlock2Ptr->frontier.curChallengeBattleNum = sPreviousDomeTourneyBattleNum;
    sPreviousDomeTourneyStateActive = FALSE;
}
...
static const u8 *const sBattleDomeOpponentStyleTexts[NUM_BATTLE_STYLES] =
{
    ...
    [DOME_BATTLE_STYLE_RARE_MOVES]      = BattleDome_Text_StyleUsesStartlingMoves,   // (fixed) Seems like the text for these two was swapped
    [DOME_BATTLE_STYLE_RARE_MOVE]       = BattleDome_Text_StyleUsesVeryRareMove,     //
...
static void InitDomeChallenge(void)
{
    ...
    sDomeTrainerBattleStylesValid = FALSE;
...
static void CreateDomeOpponentMon(u8 monPartyId, u16 tournamentTrainerId, u8 tournamentMonId, u32 otId)
{
    ...
    CreateMonWithEVSpreadNatureOTIDAbility(&gEnemyParty[monPartyId],
                                           ...
                                           gFacilityTrainerMons[DOME_MONS[tournamentTrainerId][tournamentMonId]].ability,
                                           otId);
        ...
        while (TYPE_EFFECT_ATK_TYPE(i) != TYPE_ENDTABLE)
        {
            ...
            if (TYPE_EFFECT_ATK_TYPE(i) == moveType)
            {
                ...
                if (TYPE_EFFECT_DEF_TYPE(i) == defType1)
                {
                    ...
                    if (move == MOVE_FREEZE_DRY && defType1 == TYPE_WATER)
                        multiplier = TYPE_MUL_SUPER_EFFECTIVE;
                    if ((defAbility == ABILITY_WONDER_GUARD  && multiplier == TYPE_MUL_SUPER_EFFECTIVE) || defAbility != ABILITY_WONDER_GUARD)
                        typePower = typePower * multiplier / 10;
                }
                if (TYPE_EFFECT_DEF_TYPE(i) == defType2 && defType1 != defType2)
                {
                    multiplier = TYPE_EFFECT_MULTIPLIER(i);
                    if (move == MOVE_FREEZE_DRY && defType2 == TYPE_WATER)
                        multiplier = TYPE_MUL_SUPER_EFFECTIVE;
                    if ((defAbility == ABILITY_WONDER_GUARD  && multiplier == TYPE_MUL_SUPER_EFFECTIVE) || defAbility != ABILITY_WONDER_GUARD)
                        typePower = typePower * multiplier / 10;
                }
...
static void Task_ShowTourneyInfoCard(u8 taskId)
{
    ...
    case 3:
        ...
        if (sInfoCard == NULL)
        {
            ...
            {
                if (gTasks[id].tIsPrevTourneyTree)
                    EndPreviousDomeTourneyView();
                DestroyTask(id);
            }
            DestroyTask(taskId);
...
static void DisplayTrainerInfoOnCard(u8 flags, u8 trainerTourneyId)
{
    ...
    int i, j, k;
    int trainerId = 0;
    u32 battleStyle = DOME_BATTLE_STYLE_BALANCED;
    u8 nature = 0;
    int arrId = 0;
    int windowId = 0;
    int x = 0, y = 0;
    ...
    // Battle styles are cached once per roster so card navigation stays cheap.
    battleStyle = sDomeTrainerBattleStyles[trainerTourneyId];

    // Print the trainers battle style
    textPrinter.currentChar = sBattleDomeOpponentStyleTexts[battleStyle];
    textPrinter.y = 20;
    ...
    for (i = 0; i < FRONTIER_PARTY_SIZE; i++)
    {
        for (j = 0; j < MAX_MON_MOVES; j++)
        {
            ...
            for (k = 0; k < FRONTIER_PARTY_SIZE; k++)
            {
                ...
                var = AI_TypeCalc(moveIds[i * MAX_MON_MOVES + j], gBaseStats[targetSpecies].type1, gBaseStats[targetSpecies].type2, targetAbility);
