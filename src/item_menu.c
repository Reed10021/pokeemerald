...
static const struct MenuAction sItemMenuActions[] = {
    ...
    [ITEMMENUACTION_SORT_BY_NAME] = {sMenuText_ByName,   ItemMenu_SortByName},
...
void BagMenu_MoveCursorCallback(s32 itemIndex, bool8 onInit, struct ListMenu *list)
{
    if (onInit != TRUE)
    {
        PlaySE(SE_RG_BAG_CURSOR); //SE_SELECT
        ShakeBagSprite();
    }
...
{
    u8 numDigits = (gBagPositionStruct.pocket == BERRIES_POCKET) ? 3 : BAG_ITEM_CAPACITY_DIGITS;
    ...
    if (MenuHelpers_CallLinkSomething() != TRUE && !gPaletteFade.active)
    {
        switch (GetSwitchBagPocketDirection())
        {
            ...
            default:
                ...
                {
                    if ((gBagMenu->numItemStacks[gBagPositionStruct.pocket]) <= 1) //can't sort with 0 or 1 item in bag
                    {
                        static const u8 sText_NothingToSort[] = _("There's nothing to sort!");
                        PlaySE(SE_FAILURE);
...
static u8 GetSwitchBagPocketDirection(void)
{
    ...
    {
        PlaySE(SE_RG_BAG_POCKET); //SE_SELECT
...
static const u8 sText_Number[] = _("item ID");
