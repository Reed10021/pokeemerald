...
static const u8 sBerryDescriptionPart2_Pomeg[] = _("it only grows up to five BERRIES.");
...
static u8 GetBerryCountByBerryTreeId(u8 id)
{
    ...
    u8 yield = gSaveBlock1Ptr->berryTrees[id].berryYield;

    if (currentMap == MAPSEC_ROUTE_123) // Berry Master on route 123 tends to the berries.
        yield = berry->maxYield;

    // Rain and thunder (thunder *implies* rain had or will occur, but also it's rare too) offer a flat +1. 
    if (currentWeather == WEATHER_RAIN || currentWeather == WEATHER_THUNDER_LIGHTNING)
        yield = yield + 1;
    else if(currentWeather == WEATHER_RAIN_THUNDERSTORM || currentWeather == WEATHER_DOWNPOUR ||
            currentWeather == WEATHER_ABNORMAL || currentWeather == WEATHER_SCATTERED_STORMS) // Rare weathers offer more berries.
        yield = (yield + 1) * 2;
    // Weather can't be both WEATHER_RAIN and WEATHER_RAIN_THUNDERSTORM. Max yield here is (berry->maxYield + 1) * 2.
...
{
    return GetBerryInfo(berry)->stageDuration * 30; // 60
}
